...
pagetool.view = function() {        	
	if(this.spa.selectedLines != 1) {
		...
		this.spa.isViewModal = true;  
		//console.log(this.spa.isViewModal);
	}
};
...
pagetool.save = function(savepage, name) {
	savepage.$refs[name].validate((valid) => {
        if (valid) {
        	//console.log(valid);
        	util.ajax.put(this.spa.saveurl, savepage.viewOrUpdateModel, header).then((rres) => {        		
        		if(rres.data.code===SAV_SUC || rres.data.code===UPD_SUC) {
        			this.spa.$Message.success('Success!');
        			savepage.isUpdModal=false;
                    pagetool.page({'pageSize': this.spa.pageSize, 'currentPage': this.spa.currentPage});
        		}else{
        			this.spa.$Modal.error({
                        title: '错误信息',
        	...
        	savepage.loading = false;
        	savepage.$nextTick(() => {
        		savepage.loading = true;
            });
        }
    })
};
